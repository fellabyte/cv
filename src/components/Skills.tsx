      ...
      <div className="w-full overflow-hidden relative">
        <div className="flex w-max animate-marquee space-x-10 justify-center items-center lg:gap-x-5 py-7">
          {skills.concat(skills).map((icon, index) => (
            <img key={index} src={icon.image} alt="skill-icon" className={`mx-5 rounded-lg ${icon.invert ? 'dark:invert' : ''} ${icon.big === true ? "lg:h-14 h-10 md:h-12" : icon.big === false ? "lg:h-28 h-20 md:h-24" : "lg:h-20 h-14 md:h-16"}`} />
          ))}
        </div>
      </div>
    </div>
